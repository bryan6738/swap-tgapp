...
const Deposit = ({ status }) => {
  ...
  ? status.currencies[
    status.currency_to
    ].tx_explorer.replace("{}", status.tx_to)
  : null;
  ...
  const swapStatus = (temp) => {
    if (temp == "waiting") {
      return t("Awaiting your deposit");
    } else if (temp == "confirming") {
      return t("Confirming your deposit");
    } else if (temp == "exchanging") {
      return t("Exchanging your deposit");
    } else if (temp == "sending") {
      return t("Sending to your wallet");
    } else {
      return t("Finished exchanging");
    }
  };
    ...
    <>
      <div className="bg-[#141A2E] mx-4 rounded-2xl pb-5">
        ...
        <div className="bg-[#0F75FC] mx-7 px-2 py-2 mb-5 rounded-md mt-3 flex items-center justify-between relative">
          <p className="text-white px-2 text-xs break-all">
            {address}
          </p>
          <button
      ...
      <div className="bg-white mx-4 pb-3 rounded-2xl">
        <div className="flex justify-between mx-3 my-3 pt-4">
          <div className="flex gap-x-2">
            <div
              className="h-fit p-1 mt-1 rounded-full"
              style={{ marginTop: "-1px" }}
            >
          ...
          <div className="flex gap-x-1">
            <div
              className={`h-fit p-2 rounded-full font-bold transition-colors duration-300 ${status.status == "waiting" ? "bg-blue-400" : ["confirming", "exchanging", "sending", "finished"].includes(status.status) ? "bg-blue-400" : "bg-gray-300"}`}
            >
            ...
            <div
              className={`h-fit p-2 rounded-full font-bold transition-colors duration-300 ${status.status == "exchanging" ? "bg-blue-400" : ["exchanging", "sending", "finished"].includes(status.status) ? "bg-blue-400" : "bg-gray-300"}`}
            >
            ...
            <div
              className={`h-fit p-2 rounded-full font-bold transition-colors duration-300 ${["sending", "finished"].includes(status.status) ? "bg-blue-400" : "bg-gray-300"}`}
            >
